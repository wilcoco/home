...
모든 페이지는 한국어(원문)와 영어를 함께 제공합니다.

- **자동 감지**: 방문자의 브라우저 언어가 한국어가 아니면(≈ 한국 이외 지역 접근)
  자동으로 **영어**로 표시됩니다. 한국어 브라우저는 그대로 한국어로 보입니다.
- **수동 전환**: 헤더 우측의 **`EN` / `한국어`** 토글 버튼으로 직접 전환할 수 있으며,
  선택한 언어는 브라우저에 저장(`localStorage`)되어 다음 방문·다른 페이지에서도 유지됩니다.
- **구현**: 한국어는 HTML에 원문 그대로 두고(검색엔진·비 JS 환경에 안전),
  영어는 `assets/i18n.js`의 번역 사전으로 클라이언트에서 치환합니다.
  새 문구를 추가할 때는 해당 파일의 `NODE`/`RICH` 사전에 한국어 → 영어 항목만 더하면 됩니다.
